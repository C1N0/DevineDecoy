...
class Server:
    def __init__(self, master):
        ...
        self.label = ctk.CTkLabel(
            master=self.frame, text="Login", font=("Arial", 20))
        ...
        self.login_button = ctk.CTkButton(
            master=self.frame, text="Login", command=self.login)
    ...
    def login_success(self):
        ...
        success_label = ctk.CTkLabel(
            master=self.frame, text="DevineDecoy", font=("Aerial", 20))
        success_label.pack(pady=12, padx=10)
        start_button = ctk.CTkButton(
            master=self.frame, text="Start Server", command=self.server_start)
    ...
    def server_start(self):
        ...
        self.status_label = ctk.CTkLabel(
            master=self.frame, text="Server started", font=("Aerial", 17))
        self.status_label.pack(pady=12, padx=10)
        self.show_clients_button = ctk.CTkButton(
            master=self.frame, text="Show Connected Clients", command=self.show_connected_clients)
    ...
    def show_connected_clients(self):
        if self.clients_window is None:
            ...
            self.refresh_button = tk.Button(
                self.clients_window, text="Refresh", command=self.show_connected_clients)
        ...
        for client in self.connected_clients:
            client_ip, _ = client
            client_button = Button(self.clients_window, text=str(
                client_ip), command=lambda ip=client_ip: self.handle_client_click(ip))
    ...
    def handle_client_click(self, client_ip):
        ...
        self.chat_button = tk.Button(
            self.client_window, text="Chat", command=lambda: self.open_chat_window(client_ip))
        self.chat_button.pack()
        self.send_file_button = tk.Button(
            self.client_window, text="Send File", command=lambda: self.open_send_file_window(client_ip))
    ...
    def open_chat_window(self, client_ip):
        ...
        self.send_button = tk.Button(
            self.chat_window, text="Send", command=lambda: self.send_message(client_ip))
        self.send_button.pack()
        self.paste_button = tk.Button(
            self.chat_window, text="Paste", command=self.paste)
    ...
    def open_send_file_window(self, client_ip):
        ...
        self.file_path_label = tk.Label(
            self.send_file_window, textvariable=self.file_path)
        self.file_path_label.pack()
        self.select_file_button = tk.Button(
            self.send_file_window, text="Select File", command=self.select_file)
        self.select_file_button.pack()
        self.send_file_button = tk.Button(self.send_file_window, text="Send File", command=lambda: self.send_file(
            self.client_sockets[client_ip], self.file_path.get()))
        self.send_file_button.pack()
